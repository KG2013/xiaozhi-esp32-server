...
import smhs.common.utils.AESUtils;
import smhs.common.utils.HashEncryptionUtil;
import smhs.common.utils.JsonUtils;
import smhs.modules.agent.Enums.SmhsMcpJsonRpcJson;
    ...
    public List<String> getAgentMcpToolsList(String id) {
        ...
        try {
            ...
            try (WebSocketClientManager client = WebSocketClientManager.build(
                ...
                client.sendText(SmhsMcpJsonRpcJson.getInitializeJson());
                ...
                client.sendText(SmhsMcpJsonRpcJson.getNotificationsInitializedJson());
                // 步骤3: 发送工具列表请求 - 立即发送，无需额外延迟
                log.info("发送MCP工具列表请求，智能体ID: {}", id);
                client.sendText(SmhsMcpJsonRpcJson.getToolsListJson());
