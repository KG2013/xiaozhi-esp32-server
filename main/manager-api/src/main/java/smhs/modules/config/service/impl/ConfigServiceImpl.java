package smhs.modules.config.service.impl;
...
import smhs.modules.agent.dao.AgentVoicePrintDao;
...
import smhs.modules.agent.entity.AgentVoicePrintEntity;
import smhs.modules.agent.service.AgentContextProviderService;
import smhs.modules.agent.service.AgentMcpAccessPointService;
...
import smhs.modules.agent.vo.AgentVoicePrintVO;
...
import smhs.modules.sys.dto.SysParamsDTO;
import smhs.modules.sys.service.SysParamsService;
import smhs.modules.timbre.service.TimbreService;
import smhs.modules.timbre.vo.TimbreDetailsVO;
import smhs.modules.voiceclone.entity.VoiceCloneEntity;
import smhs.modules.voiceclone.service.VoiceCloneService;
...
public class ConfigServiceImpl implements ConfigService {
    ...
    @Override
    public Map<String, Object> getAgentModelsBySsid(String ssid, Map<String, String> selectedModule) {
        // 根据MAC地址查找设备
        DeviceEntity device = deviceService.getDeviceBySsid(ssid);
        if (device == null) {
            // 如果设备，去redis里看看有没有需要连接的设备
            String cachedCode = deviceService.geCodeByDeviceId(ssid);
            if (StringUtils.isNotBlank(cachedCode)) {
                throw new RenException(ErrorCode.OTA_DEVICE_NEED_BIND, cachedCode);
            }
