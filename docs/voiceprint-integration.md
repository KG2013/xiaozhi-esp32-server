...
注意是将`smhs-esp32-server-db`下面的`expose`改成`ports`。改完后，需要重新启动。以下是重启mysql的命令：

```
# 进入你docker-compose_all.yml所在的文件夹，例如我的是smhs-server
cd smhs-server
...
## 第一步 配置接口
打开 `smhs-server/data/.config.yaml` 文件（如果没有需要创建），然后添加/修改以下内容：
