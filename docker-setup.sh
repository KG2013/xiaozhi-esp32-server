...
check_installed() {
    # 检查目录是否存在且非空
    if [ -d "/opt/smhs-server/" ] && [ "$(ls -A /opt/smhs-server/)" ]; then
        DIR_CHECK=1
    else
        DIR_CHECK=0
    fi
    ...
    # 检查容器是否存在
    if docker inspect smhs-esp32-server > /dev/null 2>&1; then
        CONTAINER_CHECK=1
    else
        CONTAINER_CHECK=0
    fi
...
if check_installed; then
    if whiptail --title "已安装检测" --yesno "检测到小智服务端已安装，是否进行升级？" 10 60; then
        ...
        docker compose -f /opt/smhs-server/docker-compose_all.yml down
        
        # 停止并删除特定容器（考虑容器可能不存在的情况）
        containers=(
            "smhs-esp32-server"
            "smhs-esp32-server-web"
            "smhs-esp32-server-db"
            "smhs-esp32-server-redis"
        )
        ...
        # 删除特定镜像（考虑镜像可能不存在的情况）
        images=(
            "ghcr.nju.edu.cn/xinnan-tech/smhs-esp32-server:server_latest"
            "ghcr.nju.edu.cn/xinnan-tech/smhs-esp32-server:web_latest"
        )
        ...
        # 备份原有配置文件
        mkdir -p /opt/smhs-server/backup/
        if [ -f /opt/smhs-server/data/.config.yaml ]; then
            cp /opt/smhs-server/data/.config.yaml /opt/smhs-server/backup/.config.yaml
            echo "已备份原有配置文件到 /opt/smhs-server/backup/.config.yaml"
        fi
        
        # 下载最新版配置文件
        check_and_download "/opt/smhs-server/docker-compose_all.yml" "https://ghfast.top/https://raw.githubusercontent.com/xinnan-tech/smhs-esp32-server/refs/heads/main/main/smhs-server/docker-compose_all.yml"
        check_and_download "/opt/smhs-server/data/.config.yaml" "https://ghfast.top/https://raw.githubusercontent.com/xinnan-tech/smhs-esp32-server/refs/heads/main/main/smhs-server/config_from_api.yaml"
        
        # 启动Docker服务
        echo "开始启动最新版本服务..."
        # 升级完成后标记，跳过后续下载步骤
        UPGRADE_COMPLETED=1
        docker compose -f /opt/smhs-server/docker-compose_all.yml up -d
    else
...
echo "开始创建安装目录..."
# 检查并创建数据目录
if [ ! -d /opt/smhs-server/data ]; then
    mkdir -p /opt/smhs-server/data
    echo "已创建数据目录: /opt/smhs-server/data"
else
    echo "目录smhs-server/data已存在，跳过创建"
fi

# 检查并创建模型目录
if [ ! -d /opt/smhs-server/models/SenseVoiceSmall ]; then
    mkdir -p /opt/smhs-server/models/SenseVoiceSmall
    echo "已创建模型目录: /opt/smhs-server/models/SenseVoiceSmall"
else
    echo "目录smhs-server/models/SenseVoiceSmall已存在，跳过创建"
fi

echo "------------------------------------------------------------"
echo "开始下载语音识别模型"
# 下载模型文件
MODEL_PATH="/opt/smhs-server/models/SenseVoiceSmall/model.pt"
...
while true; do
    ...
    if docker logs smhs-esp32-server-web 2>&1 | grep -q "Started AdminApplication in"; then
        break
    fi
    sleep 1
done
...
if [ -n "$SECRET_KEY" ]; then
    python3 -c "
import sys, yaml; 
config_path = '/opt/smhs-server/data/.config.yaml'; 
with open(config_path, 'r') as f: 
    config = yaml.safe_load(f) or {}; 
config['manager-api'] = {'url': 'http://smhs-esp32-server-web:8002/smhs', 'secret': '$SECRET_KEY'}; 
...
OTA 地址: http://$LOCAL_IP:8002/smhs/ota/\n\
视觉分析接口地址: http://$LOCAL_IP:8003/mcp/vision/explain\n\
WebSocket 地址: ws://$LOCAL_IP:8000/smhs/v1/\n\
\n安装完毕！感谢您的使用！\n按Enter键退出..." 16 70
